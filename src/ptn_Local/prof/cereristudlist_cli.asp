...
<div id="Form1" style="visibility:hidden;"
     class=TForm style="width:670px;height:350px;"
     style="left:Expression((document.body.clientWidth/2)-(this.offsetWidth/2));top:80px;">
<%CreateTableControl 12, 8, 285, Array("Last name", "First name", "Email", "Phone", "Login", "Request date"), Array(100,100,145,100,80,120), 2, "cereristudlist_dat.asp" , true, "MyTable"%>
...
<input DISABLED id="Button3" type=button value="Grant access" title="Accept selected student requests to subscribe to course"
       class=TButton style="width:110px;height:25px;"
       style="left:280px;top:308px;">
<input DISABLED id="Button4" type=button value="Deny access" title="Deny selected students access to this course"
       class=TButton style="width:110px;height:25px;"
...
<form name="FormularH" method="post" action="cereristudlist_ser.asp" target="FormReturn">
...
Sub Button4_OnClick
  ...
  RecList=GetSelectedRecords
  if RecList="" then Exit Sub
  if msgbox("Are you sure you want to deny access to course to selected students?",vbYesNo+vbQuestion,"Confirm") = vbNo then Exit Sub
